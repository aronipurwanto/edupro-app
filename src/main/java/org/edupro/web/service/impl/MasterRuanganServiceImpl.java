...
public class MasterRuanganServiceImpl implements MasterRuanganService {
    ...
    @Override
    public Optional<RuanganResponse> getById(String kode) {
         try {
             var url = Strings.concat(backEndUrl.lookupUrl(), "/" + kode);
             ResponseEntity<Response> response = restTemplate.getForEntity(url, Response.class);
             if (response.getStatusCode() == HttpStatus.OK) {
    ...
    @Override
    public Optional<RuanganResponse> update(RuanganRequest request, String kode) {
        return Optional.empty();
    }

    @Override
    public Optional<RuanganResponse> delete(String kode) {
        return Optional.empty();
    }
}
